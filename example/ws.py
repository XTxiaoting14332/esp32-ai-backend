...
class AsyncWebsocketClient:
    ...
    def write_frame(self, opcode, data=b''):
        ...
        if length < 126:
            self.sock.write(struct.pack('!BB', byte1, byte2 | length))
        elif length < (1 << 16):
            self.sock.write(struct.pack('!BBH', byte1, byte2 | 126, length))
        else:
            self.sock.write(struct.pack('!BBQ', byte1, byte2 | 127, length))

        if mask:
            # 使用随机 4 字节掩码
            mask_bits = struct.pack('!I', r.getrandbits(32))
            self.sock.write(mask_bits)
            # 高效转换：在 MicroPython 中 bytes 迭代比列表推导略快
